...
updated_file = client.file(kit_csv_file_id).update_contents(KIT_CSV_LOCAL_SAVE_PATH)

logging.info(f'KIT dashboard tracking file "{updated_file.name}" has been updated on Box.')
...
download_kit_empty_room_data_from_folder(kit_start_folder_id, kit_download_directory, client)
